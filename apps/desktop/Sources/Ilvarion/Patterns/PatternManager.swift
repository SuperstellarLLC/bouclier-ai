...
///
/// Load priority:
/// 1. User override: ~/Library/Application Support/dev.ilvarion.Ilvarion/patterns.json
...
final class PatternManager: @unchecked Sendable {
    ...
    private static func loadFromPath(_ url: URL) -> InjectionFilter? {
        ...
        print("[ilvarion] Loaded \(compiled.count) patterns from \(url.lastPathComponent) (SHA-256: \(hashPrefix)...)")
    ...
    private func handleChange() {
        ...
        if let newFilter = Self.loadFromPath(Self.userPatternsPath) {
            ...
            _filter = newFilter
            lock.unlock()
            print("[ilvarion] Patterns hot-reloaded")
            onChange?()
        }
    }
}

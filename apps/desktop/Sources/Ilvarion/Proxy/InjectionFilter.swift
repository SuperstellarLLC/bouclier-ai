...
/// Loads patterns from the shared @ilvarion/patterns package (JSON export)
...
final class InjectionFilter: Sendable {
    ...
    func scan(_ content: String) -> FilterResult {
        ...
        var allMatches: [(range: NSRange, name: String)] = []
        ...
        for variant in variants {
            ...
            for pattern in patterns where pattern.enabled {
                ...
                for match in regexMatches {
                    allMatches.append((match.range, pattern.name))
                }
            }
        }
        ...
        for (range, _) in deduped.reversed() {
            guard let swiftRange = Range(range, in: sanitized) else { continue }
    ...
    private func deduplicateOverlaps(_ matches: [(range: NSRange, name: String)]) -> [(range: NSRange, name: String)] {
        var result: [(range: NSRange, name: String)] = []
        var lastEnd = 0
    ...
    private static func loadAndVerifyPatterns() -> [FilterPattern] {
        guard let url = Bundle.main.url(forResource: "patterns", withExtension: "json"),
            ...
            print("[ilvarion] patterns.json not found, using fallback patterns")
            return fallbackPatterns()
        }

        let hash = SHA256.hash(data: data)
        let hashString = hash.map { String(format: "%02x", $0) }.joined()
        print("[ilvarion] Loaded patterns.json (SHA-256: \(hashString.prefix(16))...)")

        guard let patternSet = try? JSONDecoder().decode(PatternSetJSON.self, from: data) else {
            print("[ilvarion] Failed to decode patterns.json, using fallback patterns")
            return fallbackPatterns()
        }

        let compiled = patternSet.patterns.compactMap { FilterPattern(from: $0) }
        print("[ilvarion] Compiled \(compiled.count)/\(patternSet.patterns.count) patterns")
        return compiled
    }

...
/// Configures macOS system proxy settings to route traffic through Ilvarion.
...
enum SystemProxy {
    ...
    static func enable(port: Int) -> Bool {
        ...
        do {
            ...
            print("[ilvarion-proxy] Failed to write PAC file: \(error)")
            return false
        }

        // Get active network interface
        guard let interface = activeNetworkInterface() else {
            print("[ilvarion-proxy] No active network interface found")
            return false
        }
    ...
    static func isEnabled() -> Bool {
        ...
        do {
            ...
            return output.contains("Enabled: Yes") && output.contains("ilvarion")
        } catch {
            return false
        }
    }

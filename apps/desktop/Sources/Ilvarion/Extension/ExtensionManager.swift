...
final class ExtensionManager: NSObject, ObservableObject {
    ...
    /// macOS will prompt: "Ilvarion would like to add network configurations."
    ...
    func enableProxy() async -> Bool {
        do {
            ...
            manager.localizedDescription = "Ilvarion AI Traffic Scanner"
...
extension ExtensionManager: OSSystemExtensionRequestDelegate {
    ...
    nonisolated func requestNeedsUserApproval(_ request: OSSystemExtensionRequest) {
        NSLog("[ilvarion-ext] Extension needs user approval in System Settings")
        Task { @MainActor in
            errorMessage = "Please approve Ilvarion in System Settings > General > Login Items & Extensions"
        }
    }
    ...
    nonisolated func request(_ request: OSSystemExtensionRequest, didFinishWithResult result: OSSystemExtensionRequest.Result) {
        ...
        NSLog("[ilvarion-ext] Extension activation result: \(result)")
        Task { @MainActor in
    ...
    nonisolated func request(_ request: OSSystemExtensionRequest, didFailWithError error: Error) {
        NSLog("[ilvarion-ext] Extension error: \(error)")
        Task { @MainActor in
            extensionInstalled = false

...
struct GatewayE2ETests {
    ...
    /// THE sandwich proof. With a secret configured: a request carrying the
    /// real secret value is SCRUBBED (the upstream sees only the
    /// placeholder), and the upstream's echo of that placeholder is
    /// RESTORED so the agent receives the real value back. The model
    /// provider never sees the secret; the local agent's tool calls still
    /// work.
    @Test("Scrub→restore round-trip: model sees placeholder, agent gets real value")
    func scrubRestoreRoundTrip() async throws {
        for key in ["HTTPS_PROXY", "HTTP_PROXY", "NODE_EXTRA_CA_CERTS", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE"] {
            unsetenv(key)
        }

        let pki = try TestPKI.generate(upstreamHost: "localhost")
        let echo = try await EchoUpstream.start(certificatePEM: pki.upstreamCertPEM, keyPEM: pki.upstreamKeyPEM)
        defer { echo.shutdown() }

        // Configure a secret and turn the feature on. seedForTesting bypasses
        // Keychain; the scrub gate requires the value to be key-shaped.
        let secret = "sk_live_ROUNDTRIP_SECRET_0001"
        FeatureFlags.setTestOverride("secretInjection", true)
        SecretKeeperMonitor.resetForTesting()
        SecretStore.shared.seedForTesting(rules: [SecretRule(name: "stripe", allowedHosts: [])], values: ["stripe": secret])
        defer {
            FeatureFlags.clearTestOverrides()
        ...
        let overrides = UpstreamOverrides(
            anthropicHost: "localhost", anthropicPort: echo.port,
            openaiHost: "localhost", openaiPort: echo.port
        )
        let gateway = GatewayServer(port: 0, overrides: overrides, upstreamTrustRootsPEM: [pki.caCertPEM], onRequest: { _ in })
        let channel = try await gateway.start()
        defer { gateway.shutdown() }
        ...
        let body = #"{"messages":[{"role":"user","content":"use \#(secret) please"}]}"#
        let resp = try await GatewayDrivenClient.send(
            gatewayHost: "127.0.0.1", gatewayPort: gatewayPort,
        ...
        // 1. The upstream (model provider) must have seen the PLACEHOLDER,
        //    never the real secret.
        let observed = String(data: await echo.observedRequestBody(), encoding: .utf8) ?? ""
        #expect(!observed.contains(secret), "real secret leaked to the model: \(observed)")
        #expect(observed.contains(SecretRule.placeholder(for: "stripe")), "scrub didn't run: \(observed)")

        // 2. The agent (client) must have received the REAL value back,
        //    restored from the placeholder the echo returned.
        #expect(resp.body.contains(secret), "restore didn't put the real value back: \(resp.body)")
        #expect(!resp.body.contains(SecretRule.placeholder(for: "stripe")), "placeholder leaked to the agent: \(resp.body)")
    }

    /// Secrets configured but the request carries NO secret: the connection
    /// is restore-active, so the request goes through the parts/encoder path
    /// (not the raw relay). Prove that path is still byte-faithful — body
    /// and the cache/1M-critical headers must reach upstream unmodified.
    @Test("Restore-active connection: a clean request is still byte-faithful upstream")
    func restoreActiveCleanByteFaithful() async throws {
        for key in ["HTTPS_PROXY", "HTTP_PROXY", "NODE_EXTRA_CA_CERTS", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE"] {
            unsetenv(key)
        }
        let pki = try TestPKI.generate(upstreamHost: "localhost")
        ...
        FeatureFlags.setTestOverride("secretInjection", true)
        SecretKeeperMonitor.resetForTesting()
        SecretStore.shared.seedForTesting(rules: [SecretRule(name: "stripe", allowedHosts: [])], values: ["stripe": "sk_live_NOTINREQUEST_00000000"])
        defer { FeatureFlags.clearTestOverrides(); SecretStore.shared.clearForTesting() }
        ...
        let body = #"{"model":"claude-opus-4-8","messages":[{"role":"user","content":"no secret here"}]}"#
        let extras: [(String, String)] = [
            ("Authorization", "Bearer sk-ant-CLEAN-0000000000"),
            ("Anthropic-Version", "2023-06-01"),
            ("Anthropic-Beta", "context-1m-2025-08-07,prompt-caching-2024-07-31"),
        ]
        let resp = try await GatewayDrivenClient.send(
            gatewayHost: "127.0.0.1", gatewayPort: gatewayPort,
            method: "POST", path: "/v1/messages", body: body, extraHeaders: extras, untilEOF: true
        )
        #expect(resp.status == 200)
        let observed = String(data: await upstream.observedRequestBody(), encoding: .utf8) ?? ""
        #expect(observed == body, "parts/encoder path altered a clean body: \(observed)")
        let h = Dictionary(await upstream.observedRequestHeaders().map { ($0.0.lowercased(), $0.1) }, uniquingKeysWith: { _, l in l })
        #expect(h["anthropic-beta"] == "context-1m-2025-08-07,prompt-caching-2024-07-31")
        #expect(h["authorization"] == "Bearer sk-ant-CLEAN-0000000000")
        #expect(h["host"] == "localhost")
    }

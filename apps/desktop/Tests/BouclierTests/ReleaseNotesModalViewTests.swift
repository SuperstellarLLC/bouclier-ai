...
/// Open Settings) and that each one invokes the matching callback.
/// Without it, a refactor could silently drop the "Open Settings"
/// button and CI would still be green.
...
struct ReleaseNotesModalViewTests {
    ...
    @Test("Open Settings invokes both callbacks")
    func openSettingsInvokesBoth() throws {
        var opened = false
        var dismissed = false
        let view = ReleaseNotesModal(
            version: "1.2.3",

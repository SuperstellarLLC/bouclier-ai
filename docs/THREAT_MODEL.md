...
| J8  | **Trusting a local read admits attacker-planted local content.**                | Bounded, and the deliberate trade for J2. A `tool_result` is downgraded to `.authored` only when positively attributed (via `tool_use_id`) to `Read`/`NotebookRead` of an **absolute** path that is both (a) outside the vendored/download/temp denylist (`node_modules`, `vendor`, `Downloads`, `tmp`, …) **and** (b) **under the session's declared workspace root** — the cwd, read from the _system_ prompt only, so attacker tool content can neither add a root nor spoof one. Web/search/shell/external-MCP output, retrieved documents, reads outside the workspace, and any unattributable result stay `.untrusted`. The web→file→read laundering path is still caught at ingress. Residual: a file planted _inside_ the workspace root the agent is working in (a hostile dependency's non-vendored source, a poisoned file committed to the repo). Fallback: when no workspace root is declared (non-Claude clients) the allowlist degrades to the denylist-only decision so the FP fix isn't lost. Set `injectionTrustAuthoredReads` off (or `injectionStrict` on) to police every tool_result. |
